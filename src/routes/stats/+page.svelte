<script lang="ts">
    ...
    onMount(async () => {
        ...
        urlReady = true;
        await load();
        // the travel-date tab is independent of the main stats range;
    ...
    const TABS = ["overview", "matrix", "leadTime", "demand", "delivery", "travelDay"];
    ...
    // ---- Secured By Travel Date tab (its own date range; independent from
    // the main /stats range above because it answers a DIFFERENT question —
    // completion-date vs travel-date) ----
    ...
    // hide the whole tab so the rest of the page still works
    ...
    // grand total across all rendered rows (sum of every bucket on every row,
    // both directions merged)
    $: travelGrandTotal = travelRows.reduce((s, r) => s + rowTotal(r, ""), 0);
</script>
            ...
            <Tabs.Root bind:value={tab}>
                <div class="overflow-x-auto">
                    <Tabs.List class="w-max h-9">
                        ...
                        <Tabs.Trigger value="travelDay" class="text-xs sm:text-sm px-2.5">{$_('stats.tabs.travelDay', { locale: $lang })}</Tabs.Trigger>
                    </Tabs.List>
                </div>
                ...
                <!-- 6. Travel day × time-of-day: how many tickets were
                     SECURED for each travel date in the range (not when they
                     completed), bucketed into 6h quarters of the day. Lives
                     in its own tab because it groups by a different date
                     dimension ("how many seats did we secure for upcoming
                     travel?") than the rest of the tabs. Hidden on older zinc
                     that lacks the endpoint. -->
                <Tabs.Content value="travelDay" class="flex flex-col gap-4">
                    <Card.Root>
                        <Card.Header class="p-4 sm:p-6">
                        ...
                        <Card.Content class="px-2 sm:px-6 flex flex-col gap-3">
                            <!-- independent travel-date range (no direction
                                 filter — counts both directions merged per
                                 cell, like the other grid tabs in this page) -->
                            ...
                            {#if travelUnsupported}
                                <p class="text-sm text-muted-foreground px-2 py-6 text-center">{$_('stats.travelDate.unsupported', { locale: $lang })}</p>
                            {:else if travelLoading && travelRows.length === 0}
                                <Loader/>
                                        ...
                                        <Table.Body>
                                            {#each travelRows as r (r.date)}
                                                <Table.Row>
                                                    ...
                                                    {#each QUARTERS as q (q)}
                                                        {@const v = cellCount(r, "WToJ", q) + cellCount(r, "JToW", q)}
                                                        <Table.Cell class="px-2 py-1.5 text-right tabular-nums">{formatNumber(v, $lang)}</Table.Cell>
                                                    {/each}
                                                    <Table.Cell class="px-2 py-1.5 text-right tabular-nums font-semibold">{formatNumber(rowTotal(r, ""), $lang)}</Table.Cell>
                                                </Table.Row>
                                            {/each}
                                            <Table.Row class="border-t-2">
                                                ...
                                                {#each QUARTERS as q (q)}
                                                    {@const v = travelRows.reduce((s, r) => s + cellCount(r, "WToJ", q) + cellCount(r, "JToW", q), 0)}
