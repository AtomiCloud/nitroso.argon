<script lang="ts">
    ...
    async function promo(amount: number, desc: string) {
        ...
        ), $_('wallets.promo.errorToast', { locale: $lang })).match({
            ok: () => {
                toast.info($_('wallets.promo.successToast', { locale: $lang, values: { amount: formatMoney(amount, $lang) } }));
                invalidateAll();
            },
            err: (e) => {
                console.error(e);
...
<Dialog.Root bind:open={dialogOpen}>
    <Dialog.Trigger class="w-full max-w-80  {buttonVariants({ variant: 'default' })}">
        {$_('wallets.promo.trigger', { locale: $lang })}
    </Dialog.Trigger>
    <Dialog.Content>
        <Dialog.Header>
            <Dialog.Title>{$_('wallets.promo.title', { locale: $lang })}</Dialog.Title>
            <Dialog.Description>
                <div class="flex flex-col gap-4">
                    <p class="text-justify">
                        {$_('wallets.promo.descriptionBefore', { locale: $lang })}
                        <code class="break-all relative rounded bg-muted px-[0.3rem] py-[0.2rem] font-mono text-sm font-semibold"
                        >{user?.id?.trim() ?? 'unknown'}</code>{$_('wallets.promo.descriptionAfter', { locale: $lang })}
                    </p>
