<script lang="ts">
    ...
    async function createPassenger(c: CreatePassengerReq) {
        ...
        await toResult(() => $api.vPassengerCreate(userId, "1.0", c),
            $_('passengers.create.errorToast', { locale: $lang })).match({
            ok: ok => {
                toast.info($_('passengers.create.successToast', { locale: $lang, values: { name: ok.fullName } }));
                dialogOpen = false;
                invalidateAll();
            },
            err: (e) => {
...
<Dialog.Root bind:open={dialogOpen}>
    <Dialog.Trigger class="w-full max-w-48  {buttonVariants({ variant: 'default' })}">
        ...
        {$_('passengers.create.trigger', { locale: $lang })}
    </Dialog.Trigger>
    <Dialog.Content>
        <Dialog.Header>
            <Dialog.Title>{$_('passengers.create.title', { locale: $lang })}</Dialog.Title>
            <Dialog.Description>
                <div class="flex flex-col gap-4">
                    <p class="text-justify py-2">
                        {$_('passengers.create.subtitle', { locale: $lang })}
                    </p>
                    <Alert.Root>
        ...
        "w-full justify-start text-center font-normal",!bindDate && "text-muted-foreground")}
                    ...
                    <Button class="my-2" on:click={submit} disabled={submitting || !isValid}>
                        ...
                        {/if}
                        {$_('passengers.create.submit', { locale: $lang })}
                    </Button>
                </div>
            </Dialog.Description>
        </Dialog.Header>
